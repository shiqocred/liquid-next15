...
import { useGetListProductAbnormal } from "../_api/use-get-list-product";
...
export const Client = () => {
  ...
  const [pageAbn, setPageAbn] = useQueryState(
    "p",
    parseAsInteger.withDefault(1)
  );
  const [metaPage, setMetaPageAbn] = useState({
  ...
  const {
    data: dataAbn,
    // refetch: refetchAbn,
    isRefetching: isRefetchingAbn,
    isLoading: isLoadingAbn,
    error: errorAbn,
    isError: isErrorAbn,
    isSuccess: isSuccessAbn,
  } = useGetListAbn({
    p: pageAbn,
  });
  ...
  } = useGetListProductAbnormal({ p: pageProduct, q: searchProductValue });
  ...
  const dataRes: any = useMemo(() => {
    return dataAbn?.data.data.resource;
  }, [dataAbn]);

  const dataList: any[] = useMemo(() => {
    return dataAbn?.data.data.resource.items.data;
  }, [dataAbn]);
  ...
  const handleAddProduct = (ProductId: string, source: string) => {
    const body = {
      abnormal_document_id: dataRes?.document?.id,
      source: source,
    ...
    mutateSubmit(
      { body, id: dataRes?.document?.id },
      {
        onSuccess: (res: any) => {
          ...
          router.push(`/repair-station/abnormal/detail/${id}`);
        },
      }
    );
  };
  ...
  if (isErrorAbn && (errorAbn as AxiosError)?.status === 403) {
    return (
      ...
      <div className="flex flex-col gap-4 w-full">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href="/repair-station/abnormal">Abnormal</BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
        ...
        <div className="w-full flex gap-2 justify-start items-center pt-2 pb-1 mb-1 border-b border-gray-500">
          <Link href="/repair-station/abnormal">
            <Button className="w-9 h-9 bg-transparent hover:bg-white p-0 shadow-none">
              <ArrowLeft className="w-5 h-5 text-black" />
            </Button>
          </Link>
          <h1 className="text-2xl font-semibold">Create Abnormal</h1>
        </div>
          ...
          <div className="flex items-center justify-between pb-3 mb-5 border-gray-500 border-b w-full">
            <div className="flex items-center gap-4">
              ...
              <h5 className="font-bold text-xl">
                {dataRes?.document?.code_document_abnormal}
              </h5>
            </div>
        ...
        <div className="flex w-full bg-white rounded-md overflow-hidden shadow px-5 py-3 gap-4 flex-col">
          ...
          <DataTable
            isLoading={isRefetchingAbn || isLoadingAbn}
            columns={columnAbn}
            data={dataList ?? []}
          />
          <Pagination
            pagination={{ ...metaPage, current: pageAbn }}
            setPagination={setPageAbn}
          />
        </div>
      </div>
    </div>
