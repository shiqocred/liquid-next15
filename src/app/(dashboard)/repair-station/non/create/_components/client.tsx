...
export const Client = () => {
  ...
  const {
    data: dataNon,
    refetch: refetchNon,
    isRefetching: isRefetchingNon,
    isLoading: isLoadingNon,
    error: errorNon,
    isError: isErrorNon,
    isSuccess: isSuccessNon,
  } = useGetListNon({
    p: pageNon,
  });
  ...
  } = useGetListProductNon({ p: pageProduct, q: searchProductValue });
  ...
  const dataRes: any = useMemo(() => {
    return dataNon?.data.data.resource;
  }, [dataNon]);

  const dataList: any[] = useMemo(() => {
    return dataNon?.data.data.resource.items.data;
  }, [dataNon]);
  ...
  const handleAddProduct = (ProductId: string, source: string) => {
    const body = {
      non_document_id: dataRes?.document?.id,
      source: source,
  ...
  if (isErrorNon && (errorNon as AxiosError)?.status === 403) {
    return (
      ...
      <div className="flex flex-col gap-4 w-full">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href="/repair-station/non">Non</BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
        ...
        <div className="w-full flex gap-2 justify-start items-center pt-2 pb-1 mb-1 border-b border-gray-500">
          <Link href="/repair-station/non">
            <Button className="w-9 h-9 bg-transparent hover:bg-white p-0 shadow-none">
              <ArrowLeft className="w-5 h-5 text-black" />
            </Button>
          </Link>
          <h1 className="text-2xl font-semibold">Create Non</h1>
        </div>
          ...
          <div className="flex items-center justify-between pb-3 mb-5 border-gray-500 border-b w-full">
            <div className="flex items-center gap-4">
              ...
              <h5 className="font-bold text-xl">
                {dataRes?.document?.code_document_non}
              </h5>
            </div>
        ...
        <div className="flex w-full bg-white rounded-md overflow-hidden shadow px-5 py-3 gap-4 flex-col">
          ...
          <DataTable
            isLoading={isRefetchingNon || isLoadingNon}
            columns={columnNon}
            data={dataList ?? []}
          />
          <Pagination
            pagination={{ ...metaPage, current: pageNon }}
            setPagination={setPageNon}
          />
        </div>
      </div>
    </div>
