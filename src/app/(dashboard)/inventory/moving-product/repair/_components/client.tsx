...
import { useUnbundleRepair } from "../_api/use-unbundle-repair";
...
export const Client = () => {
  ...
  const columnListRepair: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      header: "Repair Name",
      cell: ({ row }) => (
        <div className="max-w-[500px]">{row.original.repair_name}</div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-center tabular-nums">
          {row.original.total_products.toLocaleString()}
        </div>
      ),
    },
    {
      accessorKey: "total_custom_price",
      header: "Price",
      cell: ({ row }) => formatRupiah(row.original.total_custom_price),
    },
    {
