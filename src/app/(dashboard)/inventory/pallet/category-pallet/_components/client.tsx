...
import { useCreateCategoryPallet } from "../_api/use-create-category-pallet";
import { useGetListCategoriesPallet } from "../_api/use-get-list-categories-pallet";
import { useUpdateCategoryPallet } from "../_api/use-update-category-pallet";
import { useDeleteProductCategoryPallet } from "../_api/use-delete-product-category-pallet";
import { useGetDetailCategoryPallet } from "../_api/use-get-detail-category-pallet";
...
export const Client = () => {
  ...
  const {
    ...
    error: errorCategory,
  } = useGetDetailCategoryPallet({ id: categoryId });
  ...
  const handleCreate = (e: FormEvent) => {
    e.preventDefault();
    const body = {
      name_category_palet: input.name,
      discount_category_palet: input.discount,
      max_price_category_palet: input.maxPrice,
    };
    mutateCreate(
      { body },
      {
        onSuccess: () => {
          ...
          queryClient.invalidateQueries({
            queryKey: ["list-categories-pallet"],
          });
        },
      }
    );
  ...
  useEffect(() => {
    if (isSuccessCategory && dataCategory) {
      return setInput({
        name: dataCategory.data.data.resource.name_category_palet ?? "",
        discount:
          Math.round(
            dataCategory.data.data.resource.discount_category_palet
          ).toString() ?? "0",
        maxPrice:
          Math.round(
            dataCategory.data.data.resource.max_price_category_palet
          ).toString() ?? "0",
      });
    }
  }, [dataCategory]);
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="max-w-[400px] break-all">
          {row.original.name_category_palet}
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-center tabular-nums">
          {row.original.discount_category_palet}%
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="tabular-nums">
          {formatRupiah(row.original.max_price_category_palet)}
        </div>
      ),
    },
    {
  ...
  return (
    <div className="flex flex-col items-start bg-gray-100 w-full relative px-4 gap-4 py-4">
      ...
      <div className="flex w-full bg-white rounded-md overflow-hidden shadow px-5 py-3 gap-10 flex-col">
        <h2 className="text-xl font-bold">List Categories Pallet</h2>
