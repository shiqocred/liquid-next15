...
export const Client = () => {
  ...
  const [pageQcd, setPageQcd] = useQueryState(
    "p",
    parseAsInteger.withDefault(1)
  );
  const [metaPage, setMetaPageQcd] = useState({
  ...
  const dataList: any[] = useMemo(() => {
    return dataQcd?.data.data.resource.items.data;
  }, [dataQcd]);
    ...
    <div className="flex flex-col items-start bg-gray-100 w-full relative px-4 py-4">
      ...
      <div className="flex flex-col gap-4 w-full">
        ...
        <div className="w-full flex gap-2 justify-start items-center pt-2 pb-1 mb-1 border-b border-gray-500">
          ...
          <h1 className="text-2xl font-semibold">Create QCD</h1>
        </div>
        ...
        <div className="flex w-full bg-white rounded-md overflow-hidden shadow px-5 py-3 gap-4 flex-col">
          ...
          <DataTable
            isLoading={isRefetchingQcd}
            columns={columnQcd}
            data={dataList ?? []}
          />
          <Pagination
            pagination={{ ...metaPage, current: pageQcd }}
            setPagination={setPageQcd}
          />
        </div>
      </div>
    </div>
